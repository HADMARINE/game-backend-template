...
pub fn execute_js_handler(event: String, data: String) -> Result<(), String> {
    ...
    channel.send(move |mut cx| {
        ...
        let p_event = cx.string(event.as_str());
        let p_data = cx.string(data.as_str());

        let this_val = cx.null();
        if let Err(e) = function.call(&mut cx, this_val, vec![p_event, p_data]) {
            // *error_ptr = Some(e.to_string())
            println!("Error on function call");
            eprintln!("{}", e);
        };
...
fn set_js_event_handler(mut cx: FunctionContext) -> JsResult<JsUndefined> {
    ...
    // let args = vec![cx.string("s"), cx.string("s")];
    // let null = cx.null();
    // match (*func).call(&mut cx, null, args) {
    //     Ok(_) => println!("OK"),
    ...
    match execute_js_handler(String::from("general"), String::from("init ok")) {
        Ok(_) => (),
        Err(e) => panic!(e),
    };
